...
**Gate-3a status (2026-06-09, re-swept 2026-06-10 on the data-ready cells): p_fresh is the binding input.** Error-tolerance sweep (`notebooks/gate3_tolerance.ipynb`, 36 markets/12 movies, anchor = the revised pooled +32.1¢): the PnL edge survives λ mis-estimation from **0.55× to 3×** at δ=0 (±170% random noise → 48/50 draws clear) and **δ ∈ [−0.10, 0]** of p_fresh error at m=1 — but δ=+0.05 kills the CI (random ±0.05 → 34/50; **over-estimating freshness remains the failure mode**; under-estimating is nearly free → the conservative-shade option strengthens — a 3a-era option; the shade was later measured and DROPPED, see the p_fresh paragraph). Shipped-0.2.0 proxies: Ridge λ error (m ∈ [0.62, 1.38] worst-snap) comfortably inside; `estimate_p_fresh` (~±0.03–0.05) in at −0.05, out at +0.05 → **the improvement priority inverts to p_fresh** (untouched since 0.1.x). See "GATE 3a RESULT — REVISED" in the plan.

**Gate-3b status (2026-06-10): the shipped 0.2.0 stack DOES NOT CLEAR; p_fresh isolated as THE failure input; ceiling intact.** The deployable-stack verdict ran on a re-derived **midnight-ET snap grid** (Option B; `gates/build_gate3b.py` lock chain: book LOCF → readiness/pin + guard + floor → in-grid oracle → estimator; pin 648979; 17-movie data-ready cohort; T-2d 12 mkts/10 movies, T-3d 23/13, T-4d 18/8, T-1d secondary; coverage 91.9%). Result (`notebooks/gate3b_deployable.ipynb`, pooled n=35/13 movies): **Brier diff +0.0015 [−0.0564, +0.0709], PnL +2.4¢ [−12.6, +16.8]** — neither clears. The per-cell decomposition turns 3a's prediction into a measurement: **λ fine (93% of cells in m ∈ [0.55, 3.0])** but **p_fresh out (33% in the δ-band; 42% in the kill zone δ̂ > +0.05**, horizon sign-flip: T-3d mean +0.078 / T-1d −0.063); trades with p_fresh in-band earn **+20.8¢ (75% win)** vs **−10.7¢** out-of-band. In-grid oracle on the same cells: **+31.4¢ [+22.0, +40.4]** → capture **8%**; a post-verdict −0.03 conservative shade recovers **+11.8¢ [−0.3, +26.0]** (bench re-score, fenced from the verdict). Per the locked fork the gap IS the improvement target → the p_fresh regression program ran same-day (next paragraph); the locked `gate3b_cells.csv` is the standing re-scoring bench. Full result: `plans/plan_gate3b.md` § "GATE 3b RESULT".

**p_fresh regression program (2026-06-10, same day): battery decisive; bench NO WINNER (bar-invariant); the shade — briefly the frontier — was DROPPED entirely 2026-06-11 (operator); C2′ is the sole live candidate.** Per the operator-signed design (battery decides what gets BUILT, the bench rule what SHIPS, the recorder tripwire what goes LIVE), the **Phase-0 falsification battery** ran on a fresh 135-movie / 529-row training pull (pin 653572; `gates/build_pfresh_training.py` + `gates/pfresh_lib.py`, suite 662 → 699): **the remaining-critic priors carry ~no cross-movie ranking signal** (Spearman −0.05 vs the running score's +0.874); **the subjective-intensity channel is dead** (increment −0.0002 — the score→fresh curves are step functions, 4.7% of mass in p ∈ [0.2, 0.8]) though per-critic anchors DO transfer (+0.167); **state-dependence is enormous** (visible-score coef +0.99 beyond critic identity); the bias is **behavior, not composition** (even oracle-composition keeps +0.054 of +0.074). Candidates C1′ (calibrated blend) + C2 (GLM with state terms) were built per the battery (C3 killed) and scored on the locked bench through temporal per-close fits (`notebooks/pfresh_battery.ipynb` + `pfresh_bench.ipynb`): **C1′ +7.8¢ [−5.2, +23.0], C2′ +10.0¢ [−4.4, +25.2] — both crush the shipped +2.4¢ and fix the δ̂ structure (kill-zone 42% → 27/30%), but neither beats the −0.03 shade (+11.8¢ [−0.3, +26.0]; paired −4.1¢/−1.9¢) → NO WINNER, ship nothing** (bar-invariant: both fail PnL CI-lo > 0 under ANY bar; the per-snap "T-3d strength/T-2d crater" reads are shared with the bar — post-build-review attribution catch). **Operator decision (2026-06-11): the shade is DROPPED entirely** — not a shippable candidate, not even a null baseline ("subtracting a constant isn't a principled way of modeling p_fresh; a declining-through-the-window trend should be captured in a measured/learned way" — which is exactly C2's per-snap terms). **C2′ is the sole live candidate**, run by the read-only `gates/live_scorer.py` (temporal fits, EV≥8¢ execution policy — the policy is itself bench-derived and therefore under test); future adoption = the candidate's own PnL CI clearing zero on fresh data. Permanent library shipping waits on the **out-of-sample confirmation**: the first K≈8 newly-settled live-scored movies read profitable/consistent-with-bench (the live-score log is the record; first two datapoints = the 2026-06-15 closes). Full result: `plans/plan_p_fresh_regression.md` § "RESULT" + addenda.
...
│   │                           #   book; --verify rehearsal; appends _cache/live_scores.csv (the out-of-
│   │                           #   sample confirmation record, formerly "tripwire")
