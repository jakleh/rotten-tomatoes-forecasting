...
**p_fresh regression program (2026-06-10, same day): battery decisive; bench NO WINNER (bar-invariant) — the constant shade is the current frontier.** Per the operator-signed design (battery decides what gets BUILT, the bench rule what SHIPS, the recorder tripwire what goes LIVE), the **Phase-0 falsification battery** ran on a fresh 135-movie / 529-row training pull (pin 653572; `gates/build_pfresh_training.py` + `gates/pfresh_lib.py`, suite 662 → 699): **the remaining-critic priors carry ~no cross-movie ranking signal** (Spearman −0.05 vs the running score's +0.874); **the subjective-intensity channel is dead** (increment −0.0002 — the score→fresh curves are step functions, 4.7% of mass in p ∈ [0.2, 0.8]) though per-critic anchors DO transfer (+0.167); **state-dependence is enormous** (visible-score coef +0.99 beyond critic identity); the bias is **behavior, not composition** (even oracle-composition keeps +0.054 of +0.074). Candidates C1′ (calibrated blend) + C2 (GLM with state terms) were built per the battery (C3 killed) and scored on the locked bench through temporal per-close fits (`notebooks/pfresh_battery.ipynb` + `pfresh_bench.ipynb`): **C1′ +7.8¢ [−5.2, +23.0], C2′ +10.0¢ [−4.4, +25.2] — both crush the shipped +2.4¢ and fix the δ̂ structure (kill-zone 42% → 27/30%), but neither beats the −0.03 shade (+11.8¢ [−0.3, +26.0]; paired −4.1¢/−1.9¢) → NO WINNER, ship nothing** (bar-invariant: both fail PnL CI-lo > 0 under ANY bar; the per-snap "T-3d strength/T-2d crater" reads are shared with the bar — post-build-review attribution catch). Open operator decision: ship the shade as a flagged interim (training data independently suggests s* ≈ −0.04) vs wait; either way the path is the **recorder-growth re-run + the K≈8 tripwire**. Full result: `plans/plan_p_fresh_regression.md` § "RESULT".
