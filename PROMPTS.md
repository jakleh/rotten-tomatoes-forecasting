...
1. **2026-06-12 ~00:05 ET:** `python -m gates.live_scorer --verify` (re-prove the chain minutes before money), then `--event KXRT-DIS --snap 3` and `--event KXRT-STO --snap 3` (STO at ~13 reviews will likely print `skip:features` no-trade — the stack working as designed). Sandbox-off (DB). Sizing = operator's; reads are taker-crossing with the 5¢-buffer flag printed; these two movies are the FIRST tripwire datapoints.
