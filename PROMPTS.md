...
3. **Sanity-check:** `uv sync`, then `.venv/bin/python -m pytest tests/ -q` (expect 708 green as of 2026-06-10 late PM) before touching code.
...
**SUPERSEDED SAME EVENING (operator follow-ups):** (a) the operator's split-half resilience question → battery appendix (commit `6beaaed`): **C2 beats every constant pooled AND per-snap on training OOS deviance; the fitted constant is UNSTABLE across halves (early −0.028 vs late −0.054); C2 wins all four cross-half directions** → live-use recommendation flipped to **C2′**; (b) **`gates/live_scorer.py` BUILT** (operator-commissioned; `plans/plan_live_scorer.md`; +9 tests → suite **708**): READ-ONLY, C2′ + shipped λ → `compute_edge` vs the live public book; frozen-fit/fresh-target two-worlds discipline; the stack's own skip semantics; look-ahead snap guard; `--verify` rehearsal on the gate3b hand-walk movie **PASSED with asserted anchors** (shipped p̂ drift 0.0000; C2′ 0.7156 pinned; fit 126/C=10 pinned); post-build Explore review (independently smoke-tested the live API: DIS 24 strikes / STO 12, decimal-string prices, names resolve) → 3 IMPORTANT fixes folded (C2′ anchor asserted, snap∈{1..4} guard, pool-close≤snap assert) + minors; appends `_cache/live_scores.csv` = the tripwire record.

**NEXT TASK (Thursday runbook + then mechanical):**
1. **2026-06-12 ~00:01–00:15 ET (midnight ending Thursday):** `python -m gates.live_scorer --verify` (re-prove the chain minutes before money), then `--event KXRT-DIS --snap 3` (textbook candidate: live-tracked from embargo lift, all-m/h — 168 reviews @ pin 654411; 2–3 contested strikes at probe times, re-derived live at run — trade per the reads) and `--event KXRT-STO --snap 3` (observation-grade: ~14 reviews @ pin 654411 = edge-of-support + late-tracking flag; logs a tripwire datapoint regardless). **Execution policy (trade-conditioning addendum in the plan): take only reads with printed EV ≥ ~8¢** (≈ the b≈10 raw-clearance row of the buffer sweep; low-claimed-edge trades measured −13.9¢/45%-win on the bench). Sandbox-off (DB). Sizing = operator's; these are the FIRST tripwire datapoints. Optional 06-11 00:0x ET: a logged T-4d observation run on DIS (log, don't trade — bench T-4d weak).
2. The shade-vs-C2′ SHIPPING decision (the permanent `estimate_p_fresh` replacement) stays open pending the recorder-growth re-run + tripwire results; the resilience appendix argues C2′.
