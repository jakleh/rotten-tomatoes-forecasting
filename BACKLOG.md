...
**Status:** Gate 1 DONE 2026-06-07 (market prices the observed state; stale/thin → tradeability binding — `project_gate1_findings`). Arena map DONE 2026-06-09 (`notebooks/arena_map.ipynb`): the contested∧≤10¢-spread arena exists at **T-2d..T-5d** (T-3d: 16/16 movies, median 28% of reviews still to come; ≤12h ~dead) → **Gate 2 unblocked**, scoped to those snaps vs the state-at-snap book (spread-crossing entry; cell inventory `gates/_cache/arena_spans.csv`). Next: dense-near-snap cohort guard via `db_facts` (STOP-gate) → oracle math + `compute_edge` battery → Gate 2 run. See the 2026-06-09 handoff in PROMPTS.md.
