...
**Status:** Gate 1 DONE 2026-06-07 (market prices the observed state; stale/thin → tradeability binding — `project_gate1_findings`). Arena map DONE 2026-06-09 (`notebooks/arena_map.ipynb`): the contested∧≤10¢ arena exists at **T-2d..T-5d**, center T-3d (`project_arena_map`). **Gate 2 DONE 2026-06-09 — STRONG PASS (directional)** (`notebooks/gate2_density.ipynb` STOP-gate + `notebooks/gate2_oracle.ipynb`, as_of_id=648979): oracle λ/p_fresh through `compute_edge` beats the state-at-snap book on Brier AND taker-fee PnL (T-3d +0.0775 Brier diff / +24.1¢ per contract, movie-clustered CIs clear 0; both trade sides win; lagged≈pure oracle). The market prices the state, not the flow. **Next: GATE 3** — λ/p_fresh error-tolerance sweep on the same cells; compare the shipped Ridge LOO error band (§1.1) → is 0.2.0 already inside the edge-preserving band? See the 2026-06-09 handoff in PROMPTS.md + `project_gate2_result`. Oracle support: `gates/oracle.py` + `tests/test_oracle.py` + `tests/test_edge_battery.py` (suite 98 → 628).
